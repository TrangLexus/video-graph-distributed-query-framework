#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Q4.3/Q14 — heterogeneous multi-entity behavior query."""
...
def q43_local_event_extractor(vertices: DataFrame, base_edges: DataFrame, config: Stage2Config, query_id: str) -> DataFrame:
    ...
    LocalEval must NOT decide whether the complete Q14 pattern exists. For the
...
def _q14_confidence_pass(threshold: float):
    """Disable Q14 confidence filtering.

    The current benchmark metadata does not reliably contain confidence values.
    LocalEval and GlobalEval therefore must not remove CARRIES / INTERACTS_WITH /
    USES fragments based on confidence. Keep the threshold argument only for
    backward-compatible call sites.
    """
    return F.lit(True)


def _q14_exit_role(config: Stage2Config) -> str:
    role = getattr(config, "q14_exit_person_role", "P1")
...
def _finalize_q43_candidates(raw: DataFrame, config: Stage2Config, query_id: str) -> DataFrame:
    """Finalize Q14/Q4.3 candidates using the same hard-validation contract.

    Important alignment with the previously working benchmark files:
    - Default Q14 uses Vehicle by the original person P1.
    ...
    """
    exit_role = _q14_exit_role(config)
    if exit_role == "P1":
        rpq_expression = (
        ...
        # Set Q14_QUICK_EXIT_REFERENCE=INTERACT only when the experiment requires the stricter old variant.
        if getattr(config, "q14_quick_exit_reference", "CARRY2") == "INTERACT":
    ...
    return (
        ...
        .withColumn("quick_exit_ok", (F.col("quick_exit_delay_seconds") >= F.lit(0.0)) & (F.col("quick_exit_delay_seconds") <= F.lit(float(config.quick_exit_seconds))))
        .withColumn("epsilon_time_ok", F.col("max_time_gap_seconds") <= F.lit(float(config.epsilon_time_seconds)))
        .withColumn(
            "different_interaction_location_ok",
            F.when(
                F.lit(bool(getattr(config, "q14_require_different_location", False))),
                F.when(
        ...
        .withColumn("partition_count_ok", F.col("num_partitions") >= F.lit(int(getattr(config, "q14_min_partition_count", 1))))
        .withColumn(
            "partition_trace",
        ...
        .withColumn("validation_reason", F.when(F.col("is_valid"), F.lit("PASS")).otherwise(F.lit("FAILED_Q14_CONSTRAINTS")))
        .withColumn("constraint_tags", F.concat_ws(
            ...
            F.when(F.lit(bool(getattr(config, "q14_require_different_location", False))), F.lit("different_interaction_location")),
            F.when(F.lit(int(getattr(config, "q14_min_partition_count", 1)) > 1), F.lit("cross_partition")),
        ))
        .withColumn(
            "candidate_id",
        ...
        .withColumn("_q14_tw_id_trace", F.concat_ws("->", F.col("c.tw_id"), F.col("i.tw_id"), F.col("p2c.tw_id"), F.col("u.tw_id")))
        .withColumn("_q14_location_id_trace", F.concat_ws("->", F.col("c.location_id"), F.col("i.location_id"), F.col("p2c.location_id"), F.col("u.location_id")))
        .withColumn("_q14_camera_id_trace", F.concat_ws("->", F.col("c.camera_id"), F.col("i.camera_id"), F.col("p2c.camera_id"), F.col("u.camera_id")))
        .withColumn("_q14_video_id_trace", F.concat_ws("->", F.col("c.video_id"), F.col("i.video_id"), F.col("p2c.video_id"), F.col("u.video_id")))
        .withColumn(
            "explanation_trace",
            F.concat_ws(
                " | ",
        ...
        .select(
            F.lit(query_id).alias("query_id"),
            F.lit("G4").alias("query_group"),
            F.lit("composite ERPQ / Q14-compatible").alias("logical_query_type"),
            ...
            "ts_start", "ts_end",
            F.col("_q14_tw_id_trace").alias("tw_id"),
            F.col("_q14_location_id_trace").alias("location_id"),
            F.col("_q14_camera_id_trace").alias("camera_id"),
            F.col("_q14_video_id_trace").alias("video_id"),
            "quick_exit_delay_seconds", "max_time_gap_seconds", "max_nexttw_hops",
...
def _build_q43_raw_from_atomic_events(atomic_events: DataFrame, config: Stage2Config) -> DataFrame:
    """Build Q4.3/Q14 raw candidates from semantic role events.
    ...
    - epsilon_time is not used to prune CARRY->INTERACT or INTERACT->P2_CARRY;
      these gaps are reported as metrics. The bounded continuity condition is
      enforced by time-window distance (max_nexttw_hops), matching the old
      Stage-2 files where Q14 used epsilon_tw for stitching.
    - quick_exit is measured from CARRY_2 end to USE by default, and USE must
      still occur after P2_CARRY. Set Q14_QUICK_EXIT_REFERENCE=INTERACT for
      the stricter interaction-to-exit variant.
    """
    e = _prepared_edges(atomic_events)
    carries = e.filter(F.col("relation_type") == "CARRIES").filter(_q14_confidence_pass(0.85)).dropDuplicates()
    interacts = e.filter(F.col("relation_type") == "INTERACTS_WITH").filter(_q14_confidence_pass(0.80)).dropDuplicates()
    uses = e.filter(F.col("relation_type") == "USES").filter(_q14_confidence_pass(0.85)).dropDuplicates()
    ...
    if exit_role == "P1":
        ...
        raw = (
            ...
            .filter((F.col("u.tw_num") - F.col("p2c.tw_num")).between(F.lit(0), F.lit(k)))
            .filter(F.col("u.start_epoch") >= F.col("p2c.end_epoch"))
            .filter((F.col("u.start_epoch") - F.col("p2c.end_epoch")).cast("double") <= F.lit(float(config.quick_exit_seconds)))
            .withColumn(
                "max_nexttw_hops",
                F.greatest(
                    (F.col("i.tw_num") - F.col("c.tw_num")).cast("int"),
...
def _events_from_global_graph(vertices: DataFrame, base_edges: DataFrame, config: Stage2Config, query_id: str) -> DataFrame:
    ...
    The baseline still pays the global merge and global NEXT_TW materialization cost in
    baseline.py. For the query itself, we use the same role-event semantics as Proposed
    to avoid comparing two different Q14 implementations.
    """
    return q43_local_event_extractor(vertices, base_edges, config, query_id)



def _maybe_print_q43_diagnostics(atomic_events: DataFrame, config: Stage2Config, label_prefix: str = "q14") -> None:
    """Print Q14 stage-by-stage diagnostic counts when enabled.
    ...
    """
    if not bool(getattr(config, "q14_diagnostics_enabled", False)):
        return
    try:
        e = _prepared_edges(atomic_events)
        carry = e.filter(F.col("relation_type") == "CARRIES").filter(_q14_confidence_pass(0.85)).dropDuplicates()
        inter = e.filter(F.col("relation_type") == "INTERACTS_WITH").filter(_q14_confidence_pass(0.80)).dropDuplicates()
        uses = e.filter(F.col("relation_type") == "USES").filter(_q14_confidence_pass(0.85)).dropDuplicates()
        print(f"[Q14-DIAG] {label_prefix}.num_q14_role_carries = {carry.count()}")
        print(f"[Q14-DIAG] {label_prefix}.num_q14_role_interacts = {inter.count()}")
        print(f"[Q14-DIAG] {label_prefix}.num_q14_role_uses = {uses.count()}")
        ...
        print(f"[Q14-DIAG] {label_prefix}.num_q14_join_ci_binding = {ci_binding.count()}")
        ...
        print(f"[Q14-DIAG] {label_prefix}.num_q14_join_ci_temporal_tw = {ci_temporal.count()}")
        ...
        print(f"[Q14-DIAG] {label_prefix}.num_q14_join_ci_spatial_different_location = {ci_spatial_diff.count()}")
        ...
        print(f"[Q14-DIAG] {label_prefix}.num_q14_join_cip = {cip.count()}")

        if _q14_exit_role(config) == "P1":
        ...
        print(f"[Q14-DIAG] {label_prefix}.num_q14_join_full_person = {full_person.count()}")
        ...
        full_temporal = (
            ...
            .filter((F.col("u.tw_num") - F.col("p2c.tw_num")).between(F.lit(0), F.lit(k)))
            .filter(F.col("u.start_epoch") >= F.col("p2c.end_epoch"))
            .filter(quick_delay <= F.lit(float(config.quick_exit_seconds)))
        )
        print(f"[Q14-DIAG] {label_prefix}.num_q14_join_full_temporal_quick_exit = {full_temporal.count()}")

        full_partition = full_temporal.withColumn(
            "_num_partitions",
            F.size(F.array_distinct(F.array("c.partition_id", "i.partition_id", "p2c.partition_id", "u.partition_id"))),
        ).filter(F.col("_num_partitions") >= F.lit(int(getattr(config, "q14_min_partition_count", 1))))
        print(f"[Q14-DIAG] {label_prefix}.num_q14_join_full_partition_valid = {full_partition.count()}")
    except Exception as exc:
        print(f"[Q14-DIAG] {label_prefix}.diagnostics_failed = {repr(exc)}")
...
def q43_baseline_global_evaluator(
    ...
    """Baseline evaluator: global graph first, same Q4.3/Q14 logic as Proposed.

    Baseline still performs Union/Merge and global NEXT_TW materialization before this
    evaluator is called. The evaluator itself uses the same bounded role-event query
    as Proposed so that correctness comparison is based on the same witness logic.
    """
    ...
    witnesses = (
        ...
        .withColumn("witness_type", F.lit("q14_handover_quick_exit"))
...
def q43_distributed_atomic_evaluator(
    ...
    """Proposed evaluator: LocalEval fragments + same bounded Q4.3/Q14 logic."""
...
def q43_candidate_upper_bound_without_nexttw(atomic_events: DataFrame, config: Stage2Config, query_id: str) -> DataFrame:
    ...
    This function intentionally does NOT assemble the full Q14 pattern. It only
...
Q43_SPEC = QuerySpec(
    ...
    aliases=("Q14",),
    query_group="G4",
    query_name="Heterogeneous Multi-entity Behavior / Q14-compatible Query",
    logical_query_type="boundary-fragment composite ERPQ / Q14-compatible",
    rpq_expression=(
        "CARRIES(P1,T) · NEXT_TW_PERSON* · INTERACTS_WITH(P1,P2) · "
        "NEXT_TW_PERSON* · CARRIES(P2,T) · NEXT_TW_PERSON* · USES(P1,VH)"
    ),
    physical_plan="boundary_fragment_localeval_then_globaleval_stitching",
    witness_type="q14_handover_quick_exit",
    compare_columns=COMPARE_COLUMNS_Q43,
