...
# ============================================================
# VideoGraphDB C2 - Profiling Benchmark - One query
#
# Purpose:
#   Run the materialized phase-boundary profiling benchmark for
#   one selected query over the configured HDFS datasets.
#
# Important:
#   This script is for PROFILING / phase-cost attribution only.
#   It calls run_experiment.py with --mode profiling.
#   Results must NOT be reported as official runtime.
#
# Usage:
...
#   DATASETS=dataset_1m PROFILE_REPEATS=1 ./scripts/run_one_query_profile.sh Q2.3
# ============================================================
...
LOG_FILE="${LOG_DIR}/run_one_query_profile_${QUERY_TAG}_${RUN_ID}.log"
STATUS_FILE="${STATUS_DIR}/run_one_query_profile_status_${QUERY_TAG}_${RUN_ID}.csv"

# Default: run a small profiling test first. Override DATASETS for larger runs.
...
{
  echo "============================================================"
  echo "VideoGraphDB one-query profiling benchmark"
  echo "Run ID:       ${RUN_ID}"
...
PY
  ...
  echo
  echo "Step 3/3 - Profiling benchmark"
  CMD=(
  ...
  echo
  echo "============================================================"
  echo "Completed one-query profiling benchmark"
