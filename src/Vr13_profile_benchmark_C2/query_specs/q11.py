...
from query_specs.common import QuerySpec, prepared_edges
    ...
    def orient(entity_col: str, context_col: str) -> DataFrame:
        return (
            ...
            .select(
                ...
                F.regexp_extract(
                    F.coalesce(F.col("e.tw_id"), F.col("v.entity_vertex_tw_id")).cast("string"),
                    r"(\d+)", 1
                ).cast("int").alias("tw_num"),
...
Q11_SPEC = QuerySpec(
    ...
    physical_plan=(
        "same_entity_observation_extraction_then_bounded_tw_ordering_"
        "and_trajectory_stitching"
    ),
    witness_type="trajectory",
    compare_columns=COMPARE_COLUMNS_Q11,
