...
OUTPUT_BASE="${OUTPUT_BASE:-${PROJECT_DIR}/benchmark_outputs_vr15/profiling_runtime_only/${RUN_ID}}"
mkdir -p "${OUTPUT_BASE}"
FAILED=()
for QUERY_ID in "${QUERIES[@]}"; do
  echo "===== Profiling runtime-only ${QUERY_ID} ====="
  if OUTPUT_ROOT="${OUTPUT_BASE}/${QUERY_ID//./_}" \
     DATASETS="${DATASETS:-dataset_1m}" PROFILE_REPEATS="${PROFILE_REPEATS:-1}" \
     "${PROJECT_DIR}/scripts/run_one_query_profile_runtime_only.sh" "${QUERY_ID}"; then
    echo "PASS ${QUERY_ID}"
  else
    FAILED+=("${QUERY_ID}")
  fi
