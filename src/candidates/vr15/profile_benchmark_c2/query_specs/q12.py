...
- Avoid APPEAR × CARRY explosion by selecting the nearest valid APPEAR for each
  CARRY event.
- Bound temporal joins by both observed_max_time_window_gap and max_time_gap_seconds.
...
Q12_SPEC = QuerySpec(
    query_id="Q1.2", query_group="G1",
    query_name="Carry–Interact–Vehicle-use Sequence",
    logical_query_type="Composite ERPQ",
    rpq_expression="APPEAR(P) · NEXT_TW_PERSON* · CARRIES(P,T) · NEXT_TW_PERSON* · INTERACTS_WITH(P,P2) · NEXT_TW_PERSON* · USES(P,VH)",
    logical_query_description="Chuỗi hành vi có thứ tự gồm xuất hiện, mang vật, tương tác và sử dụng phương tiện.",
    baseline_physical_plan="GLOBAL_MERGE_THEN_ORDERED_ROLE_BINDING_JOIN",
    proposed_physical_plan="PARTITION_ROLE_LOCALEVAL_THEN_ORDERED_FRAGMENT_STITCHING",
    witness_type="composite_pattern",
    correctness_compare_columns=COMPARE_COLUMNS_Q12,
    ...
    automaton_transitions=(("q0","APPEAR","q1"),("q1","CARRY","q2"),("q2","INTERACT","q3"),("q3","USE","q4")),
    required_fragment_roles=("APPEAR","CARRY","INTERACT","USE"),
    boundary_eligible_roles=("APPEAR","CARRY","INTERACT","USE"),
    stitch_mode="ORDERED_BEHAVIOR_CHAIN", stitch_key_fields=("person_id",),
    stitch_role_order=("APPEAR","CARRY","INTERACT","USE"),
    boundary_interfaces=(("APPEAR","CARRY",("person_id",),("person_id",),"FORWARD","q1","q1"),("CARRY","INTERACT",("person_id",),("person_id",),"FORWARD","q2","q2"),("INTERACT","USE",("person_id",),("person_id",),"FORWARD","q3","q3")),
    hard_constraint_names=("pattern_coverage","same_person","temporal_order","bounded_continuity","binding_consistency"),
    candidate_dedup_columns=("candidate_id",),
    witness_dedup_columns=tuple(COMPARE_COLUMNS_Q12),
