...
# ============================================================
# VideoGraphDB C1 - Official Benchmark - Multiple queries
#
# Purpose:
#   Run the official lazy/end-to-end runtime benchmark for selected
#   VideoGraphDB queries over the configured HDFS datasets.
#
# Important:
#   This script is for OFFICIAL paper/luận án runtime measurement.
#   It calls run_experiment.py with --mode official.
#   It must NOT use --mode profiling or profiling-only diagnostic
#   materialization/count actions.
#
# Behavior:
...
#   ./scripts/run_multi_queries_official.sh
# ============================================================
...
OFFICIAL_BASE="${OUTPUT_BASE}/official/${RUN_ID}"
LOG_DIR="${OUTPUT_BASE}/logs"
STATUS_DIR="${OUTPUT_BASE}/status"

mkdir -p "${OFFICIAL_BASE}" "${LOG_DIR}" "${STATUS_DIR}"

LOG_FILE="${LOG_DIR}/run_multi_queries_official_${RUN_ID}.log"
STATUS_FILE="${STATUS_DIR}/run_multi_queries_official_status_${RUN_ID}.csv"
...
OFFICIAL_REPEATS="${OFFICIAL_REPEATS:-${TIMING_REPEATS:-3}}"
...
{
  echo "============================================================"
  echo "VideoGraphDB official multi-query benchmark"
  echo "Run ID:       ${RUN_ID}"
  echo "Project dir:  ${PROJECT_DIR}"
  echo "Output base:  ${OUTPUT_BASE}"
  echo "Output dir:   ${OFFICIAL_BASE}"
  echo "Queries:      ${QUERIES[*]}"
  echo "Datasets:     ${DATASETS}"
  echo "Repeats:      ${OFFICIAL_REPEATS}"
  echo "Mode:         official"
  echo "Benchmark:    official lazy/end-to-end paper runtime"
...
PY
  ...
  for QUERY_ID in "${QUERIES[@]}"; do
    ...
    QUERY_OUTPUT_ROOT="${OFFICIAL_BASE}/query_outputs_official_${QUERY_TAG}"
    ...
    echo
    echo "============================================================"
    echo "Official benchmark for query: ${QUERY_ID}"
...
PY
    ...
    if python3 run_experiment.py \
      ...
      echo "Official benchmark PASS: ${QUERY_ID}"

    else
      FINISHED_AT="$(date '+%Y-%m-%d %H:%M:%S')"
      echo "${QUERY_ID},FAIL,${QUERY_OUTPUT_ROOT},${STARTED_AT},${FINISHED_AT}" >> "${STATUS_FILE}"
      echo "Official benchmark FAIL: ${QUERY_ID}"
  ...
  if [[ "${OVERALL_STATUS}" -eq 0 ]]; then
    echo "All official benchmarks completed successfully"
  else
    echo "Official benchmark completed with failed queries"
